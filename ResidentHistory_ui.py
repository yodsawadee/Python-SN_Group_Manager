...
class ResidentHistory(QMainWindow):
    def __init__(self):
        QMainWindow.__init__(self, None)
        
        loader = QUiLoader()
        form = loader.load("UI/uiResidentHistory.ui", None)
        ...
        self.history = self.db.allresidenthistory()

        self.showInfo(self.history)

    def showInfo(self, History):
        self.tableWidget.setRowCount(len(self.history))
        self.tableWidget.setColumnCount(11)
        self.layout.addWidget(self.tableWidget, 0,0)
        row = 0
        for eachRes in History:
