...
class Checkout(QMainWindow):
    def __init__(self,menu):
        ...
        loader = QUiLoader()
        form = loader.load("UI/uiCheckout.ui", None)
        ...
        self.checkout_btn = form.findChild(QPushButton, "checkout_btn")
    ...
    def checkout(self):
        if(self.db.IsAvailable(self.roomNo)):
            self.popUPRoomAvailable()
        else:
            self.popUPRoomNotAvailable()

    def popUPRoomNotAvailable(self):
        layout = QVBoxLayout()
        label = QLabel("Are you sure")
        layout.addWidget(label)
        
        no_button = QPushButton("No")
        no_button.clicked.connect(lambda: self.confirm("No"))

        yes_button = QPushButton("Yes")
        yes_button.clicked.connect(lambda: self.confirm("Yes"))
        
        layout.addWidget(no_button)
        layout.addWidget(yes_button)
        self.confirm_dialog.setLayout(layout)
        self.confirm_dialog.show()

    def confirm(self, ans):
        if(ans == "Yes"):
            self.db.checkOut(self.roomNo)
            self.checkout_btn.setEnabled(False)
    ...
    def popUPRoomAvailable(self):
        dialog = QDialog(self)
        layout = QVBoxLayout()
        label = QLabel("Room availabel, you can not check out!")
        layout.addWidget(label)
